        ...
        <AnimatePresence>
          {mega && (
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.25 }}
              ...
              <div className="px-8 py-8 max-w-7xl mx-auto grid grid-cols-6 gap-6">
                {navCategories.map((c) => (
                  <div key={c.name} className="flex flex-col">
                    <Link to={`/shop?cat=${encodeURIComponent(c.name)}`} className="font-display font-bold text-sm uppercase tracking-wide text-ink-50 hover:text-blaze-400 transition-colors border-b border-ink-800 pb-2 mb-3">{c.name}</Link>
                    ...
                    <Link to={`/shop?cat=${encodeURIComponent(c.name)}`} className="inline-flex items-center gap-1.5 pt-3 font-mono text-[10px] tracking-[0.2em] uppercase text-blaze-500 hover:text-blaze-400">All {c.name} <Ic.arrow className="w-3 h-3" /></Link>
                  </div>
                ))}
                <Link to="/product/raspberry-noir" className="relative clip-tile overflow-hidden group border border-ink-700/60 bg-ink-850 flex flex-col justify-end">
  ...
  return (
    <>
      <motion.button whileTap={{ scale: 0.9 }} onClick={() => setOpen(!open)} aria-label="Support chat"
        className={`fixed left-5 z-[75] w-13 h-13 p-3.5 bg-cobalt-500 hover:bg-cobalt-400 text-ink-50 clip-btn shadow-lift transition-all ${consent === "pending" ? "bottom-56" : "bottom-24 md:bottom-6"}`}>
      ...
      <AnimatePresence>
        {open && (
          <motion.div initial={{ opacity: 0, y: 20, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 20, scale: 0.96 }}
            className="fixed bottom-36 md:bottom-22 left-5 z-[75] w-[min(92vw,360px)] h-105 bg-ink-900 border border-ink-700 shadow-lift flex flex-col clip-tile">
            <div className="px-4 py-3 bg-ink-850 border-b border-ink-700 flex items-center justify-between">
              ...
              <button onClick={() => setOpen(false)} aria-label="Close chat" className="text-ink-400 hover:text-ink-100 p-1 transition-colors">
...
export function WhatsAppWidget() {
  ...
  return (
    <>
      ...
      <div className={`fixed right-4 md:right-6 z-[76] flex items-center gap-3 transition-all ${consent === "pending" ? "bottom-56" : "bottom-24 md:bottom-6"}`}>
        {!open && (
          <motion.div
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className={`fixed right-4 md:right-6 z-[77] w-[min(92vw,380px)] bg-ink-900 border border-ink-700 shadow-lift flex flex-col clip-tile overflow-hidden ${consent === "pending" ? "bottom-68" : "bottom-40 md:bottom-22"}`}
          >
