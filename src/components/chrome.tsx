...
import { Ic, ImgX, Modal, Qty } from "./ui";
    ...
    <Modal open={!!mode} onClose={onClose}>
      <div className="grid md:grid-cols-[1.1fr_1.4fr]">
        <div className="relative hidden md:flex flex-col justify-between p-8 bg-ink-850 overflow-hidden">
          ...
          <div className="relative flex items-center gap-2 text-blaze-500"><Ic.bolt className="w-7 h-7" /><span className="font-display font-extrabold tracking-wide">VOLTA</span></div>
          <div className="relative">
            <p className="font-display text-2xl font-bold leading-tight uppercase">Member<br />access</p>
            <p className="text-sm text-ink-300 mt-3 leading-relaxed">Order tracking, wishlists, one-tap checkout and early drops.</p>
          </div>
          <p className="relative font-mono text-[10px] text-ink-500 tracking-widest">JWT SESSIONS · OTP VERIFIED · GDPR READY</p>
        </div>
          ...
          {view === "form" && (
            <>
              ...
              <div className="mt-5 p-3 border border-dashed border-ink-600 font-mono text-[10px] text-ink-400 leading-relaxed">
                DEMO — customer: user@volta.shop / demo123<br />DEMO — admin: admin@volta.shop / demo123
              </div>
            </>
          )}
          {view === "otp" && (
        ...
        <div className="flex items-center justify-between px-4 sm:px-6 lg:px-10 h-16">
          <div className="flex items-center gap-8">
            <Link to="/" className="flex items-center gap-2 text-ink-50">
              <Ic.bolt className="w-7 h-7 text-blaze-500" />
              <span className="font-display font-extrabold text-lg tracking-wide">VOLTA</span>
            </Link>
          ...
          {mega && (
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.25 }}
              ...
              <div className="px-10 py-10 grid grid-cols-[repeat(5,1fr)_1.4fr] gap-8">
                ...
                <Link to="/product/velocity-x1-cobalt" className="relative clip-tile overflow-hidden group border border-ink-700/60 bg-ink-850">
                  <div className="absolute inset-0 grid-lines opacity-50" />
                  <MegaFeatured />
                  <div className="absolute inset-x-0 bottom-0 p-5 bg-gradient-to-t from-ink-950 via-ink-950/60 to-transparent">
                    <p className="font-mono text-[10px] tracking-[0.25em] text-blaze-400">LIMITED /500</p>
                    <p className="font-display font-bold mt-1 group-hover:text-blaze-400 transition-colors">Velocity X1 Cobalt</p>
                  </div>
                </Link>
              </div>
            </motion.div>
...
function MegaFeatured() {
  ...
  const p = products.find((x) => x.id === "velocity-x1-cobalt");
  if (!p) return null;
  return <ImgX src={p.img} alt={p.name} className="absolute inset-0 w-full h-full object-cover transition-transform duration-700 hover:scale-105" style={{ filter: p.imgFilter }} />;
}
...
/* ================= chat widget ================= */
const CANNED = [
  "Got it — checking that for you now. Orders ship within 24h and you'll get a tracking push notification.",
  "Our sizing runs true; if you're between sizes on the X1, go half a size up.",
  "Returns are free for 30 days — start one from Account → Orders and the label arrives by email instantly.",
  "The code WELCOME10 gives 10% off your first order — apply it in the cart.",
];
        ...
        {open && (
          <motion.div initial={{ opacity: 0, y: 20, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 20, scale: 0.96 }}
            ...
            <div className="px-4 py-3 bg-ink-850 border-b border-ink-700 flex items-center gap-3">
              ...
              <div><p className="font-display text-sm font-bold">VOLTA Support</p><p className="font-mono text-[10px] text-volt-400">● online — replies in ~1 min</p></div>
            </div>
...
const POLICIES: Record<string, { title: string; body: string[] }> = {
  privacy: { title: "Privacy Policy", body: ["VOLTA collects only the data required to fulfil orders: contact details, addresses and encrypted payment tokens. We never sell personal data.", "Data is stored encrypted at rest (AES-256) and in transit (TLS 1.3). You may request a full export or deletion of your data at any time from Account → Settings, per GDPR Article 17.", "Analytics are aggregated and anonymized. Cookie preferences can be changed at any time by clearing site data."] },
  ...
  gdpr: { title: "GDPR Compliance", body: ["Right to access, rectify and erase: email privacy@volta.shop and we respond within 30 days.", "Cookie consent is recorded and respected across sessions — declining disables all non-essential tracking.", "Our data processing agreements with payment providers (Stripe, Razorpay, PayPal) are fully PCI-DSS compliant."] },
};
