...
export default function Home() {
  const { products, categories, settings, fmt } = useStore();
  const { t } = useStore();
  ...
  const featured = (settings.featuredProductIds && settings.featuredProductIds.length > 0)
    ...
    : products.filter((p) => p.featured || p.tag).slice(0, 4);
  ...
  return (
    <div className="overflow-x-clip">
          ...
          <div className="relative max-w-7xl mx-auto px-6 lg:px-10 py-24 grid lg:grid-cols-[1.15fr_1fr] gap-10 items-center w-full">
            <div>
              <p className="inline-flex items-center gap-2.5 font-mono text-[11px] tracking-[0.28em] text-blaze-400 clip-tag border border-blaze-500/40 bg-blaze-500/10 px-4 py-2 anim-fade-up">
                <span className="w-1.5 h-1.5 bg-emerald-400 pulse-dot rounded-full" /> {settings.hero?.kicker || "100% PURE EGGLESS BAKERY · 30-45 MIN EXPRESS DELIVERY"}
              </p>
              ...
              <p className="text-ink-300 text-lg max-w-xl mt-7 leading-relaxed anim-fade-up" style={{ animationDelay: "240ms" }}>
              ...
              <div className="flex flex-wrap items-center gap-4 mt-9 anim-fade-up" style={{ animationDelay: "320ms" }}>
                <Link to={settings.hero?.ctaLink || "/shop"} className="clip-btn group bg-blaze-500 hover:bg-blaze-400 text-ink-50 font-mono text-sm tracking-[0.2em] uppercase px-8 py-4 transition-all hover:shadow-glow flex items-center gap-3">
                  {settings.hero?.ctaText || t("shopNow")} <Ic.arrow className="w-4 h-4 transition-transform group-hover:translate-x-1" />
                </Link>
                <Link to={settings.hero?.secCtaLink || "/shop?cat=Cakes"} className="clip-btn border border-ink-600 hover:border-ink-300 hover:text-ink-50 text-ink-200 font-mono text-sm tracking-[0.2em] uppercase px-8 py-4 transition-colors">
                  {settings.hero?.secCtaText || t("explore")}
                </Link>
              </div>
              ...
              <div className="md:hidden anim-float">
                <div className="relative bg-ink-850 clip-tile border border-ink-700 overflow-hidden max-w-sm mx-auto">
                  ...
                  <div className="absolute inset-x-0 bottom-0 p-5 bg-gradient-to-t from-ink-950 to-transparent flex justify-between items-end">
                    <div>
                      <p className="font-display font-bold">{heroProduct?.name}</p>
                      <p className="font-mono text-sm text-blaze-400 mt-1">{fmt(heroProduct?.price ?? 0)}</p>
                    </div>
                    <Link to={`/product/${heroProduct?.id}`} className="clip-tag bg-blaze-500 p-3 text-ink-50"><Ic.arrow className="w-4 h-4" /></Link>
                  </div>
                </div>
              </div>
            </div>
            ...
            {[0, 1].map((k) => (
              <div key={k} className="flex items-center">
                {[
                  ...
                  "Baked Fresh Daily in Faridabad & Noida Hubs",
                  "Customized Designer Cakes & Photos",
                  "Handwritten Message Cards Included",
                  "Belgian Chocolate & Gourmet Berries"
                ].map((s) => (
      ...
      {/* ============ NCR EXPRESS HUBS (SEO & LOCAL VISIBILITY) ============ */}
      {homeSections.ncrHubs !== false && hubs.length > 0 && (
        <section className="max-w-7xl mx-auto px-6 lg:px-10 py-16">
          <div className="border border-ink-700/60 bg-ink-900/60 backdrop-blur-md p-6 sm:p-8 clip-tile">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-ink-800 pb-6">
              <div>
                <span className="inline-flex items-center gap-2 text-[10px] font-mono tracking-widest uppercase text-emerald-400 bg-emerald-500/10 border border-emerald-500/30 px-3 py-1 rounded">
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-ping" /> Delhi NCR Hyperlocal Delivery Network
                </span>
                <h2 className="font-display font-bold text-2xl sm:text-3xl uppercase mt-3">Express Cake Delivery In Your City</h2>
                <p className="text-sm text-ink-300 mt-1 max-w-2xl">
                  Order 100% eggless gourmet cakes delivered within 30-45 minutes with live temperature-controlled tracking.
                </p>
              </div>
              <div className="flex items-center gap-3">
                <a
                  ...
                  className="clip-tag px-4 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-ink-50 font-mono text-xs tracking-wider uppercase inline-flex items-center gap-2 transition-colors"
                >
                  <span>WhatsApp 30-Min Fast Order</span>
                </a>
              </div>
            </div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <span className="font-mono text-[9px] uppercase tracking-widest text-blaze-400 font-bold">⚡ {h.time}</span>
                    ...
                    <div className="mt-3 pt-3 border-t border-ink-800/80 flex items-center justify-between text-[10px] font-mono">
                      <span className="text-ink-400">Min Order: ₹{h.minOrder}</span>
                      <Link to={`/shop?hub=${h.id}`} className="text-blaze-400 hover:underline font-bold">Order →</Link>
                    </div>
                  </div>
                );
              })}
        ...
        <section ref={plx} className="relative h-[80vh] min-h-[520px] overflow-hidden noise">
          ...
          <div className="relative h-full max-w-7xl mx-auto px-6 lg:px-10 flex flex-col justify-center">
            <div data-plx-text className="will-change-transform">
              <p className="font-mono text-[11px] tracking-[0.3em] text-blaze-400 mb-4">100% PURE VEGETARIAN · 72-HOUR COLD FERMENTATION</p>
              ...
              <Reveal delay={220}>
                <p className="text-ink-200 max-w-lg mt-6 text-lg leading-relaxed">
                  Every sponge rested. Every coulis slow-set. Every cookie dough cold-fermented for 72 hours. We don't bake in advance — we bake to order for Delhi NCR's grandest celebrations.
                </p>
              </Reveal>
              <Reveal delay={320}>
                <Link to="/blog" className="clip-tag inline-flex items-center gap-3 mt-8 border border-ink-100/40 hover:border-blaze-500 hover:text-blaze-400 px-6 py-3 font-mono text-xs tracking-[0.2em] uppercase transition-colors text-ink-100">
                  Read our artisan bake notes <Ic.arrow className="w-4 h-4" />
                </Link>
              </Reveal>
            </div>
          </div>
      ...
      {homeSections.featured !== false && (
        <section className="max-w-7xl mx-auto px-6 lg:px-10 pb-24">
          <SectionHead kicker="02 — Signature Selection" title={t("featured")} link={<Link to="/shop" className="clip-tag border border-ink-600 hover:border-blaze-500 hover:text-blaze-400 px-5 py-2.5 font-mono text-xs tracking-[0.15em] uppercase inline-flex items-center gap-2 transition-colors">{t("viewAll")} <Ic.arrow className="w-3.5 h-3.5" /></Link>} />
        ...
        <section className="relative border-y border-ink-800 bg-ink-900 noise">
          ...
          <div className="relative max-w-7xl mx-auto px-6 lg:px-10 py-24 grid lg:grid-cols-2 gap-14">
            <div className="lg:sticky lg:top-32 self-start">
              ...
              <p className="text-ink-300 mt-6 max-w-md leading-relaxed">Four numbers decide whether a recipe earns the CakeUrban label. Miss one, and it goes back to the bench. 100% pure vegetarian, 0 preservatives, pure Belgian cocoa.</p>
            ...
            <div className="space-y-5">
              {[
                ["01", "72 h", "Cold-fermented dough", "Our choc-chip and cookie dough rests three full days. Hydration and deep toffee notes can't be rushed."],
                ["02", "72%", "Belgian couverture chocolate", "The Noir and Fudge lines run exclusively on single-origin dark chocolate — zero compound, zero artificial essences."],
                ["03", "100%", "Pure Eggless & Veg", "Crafted in a dedicated vegetarian bakery kitchen with certified hygiene and no cross-contamination."],
                ["04", "30-45m", "Express NCR delivery", "Chilled express riders deliver across Faridabad, Noida, Gurgaon and Delhi before your celebrations begin."],
      ...
      {homeSections.reviews !== false && (
        <section className="max-w-7xl mx-auto px-6 lg:px-10 py-24">
          <SectionHead kicker="04 — Sweet words" title="2,480+ verified Google reviews" />
          <div className="grid md:grid-cols-3 gap-5 items-start">
            <Reveal className="md:mt-8">
              <div className="border border-ink-700/60 bg-ink-850 p-8 clip-tile text-center">
                ...
                <p className="font-mono text-[11px] tracking-[0.2em] text-ink-400 uppercase">Average across Delhi NCR orders</p>
              </div>
            </Reveal>
            {reviews.map((r, i) => (
